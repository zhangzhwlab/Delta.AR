	...
	function configView(){
		...
		$("#trackid input[type='checkbox']").each(function(){
							if ($(this).is(":checked")) {
									...
									//cncb tracks
									if(arrys[0].indexOf('cncb') > -1){
										//get tracks configure json format
										var json = "/deltaar/json/cncb/"+arrys[1]+".json";
										console.log(json);
										var params={}
										$.ajax({
											url : json,
											...
											success : function(result){
												...
												if(result.length>0){
													pcount ++;
													result = result[0];
													var tmp_en= JSON.stringify(result);
			...
			if(qcount > 1){
											...
											if(isdrawtrack != null ){
												...
												if(arrys[0].indexOf('cncb') > -1){
													...
													if(res == 0 ){
														...
														var url="http://"+window.location.host+"/jbrowse/delta_index.html?data="+conf+"&nav=1&overview=1&menu=1&loc="+loc+"&tracks="+gtrack+"&hololensid="+sid+"&model="+model+"&binsize="+bin+"&organism="+conf;		
													//	window.location.href="/deltaar/pages/visualization/physical_view.jsp?conf="+conf+"&loc="+loc+"&tracks="+track+"&model="+model+"&bin="+bin+"&zoom=146&hololensid="+sid;
														...
														if(jsonstr!="" && jsonstr.endsWith(",") ==true){
				...
				url="http://"+window.location.host+"/jbrowse/delta_index.html?data="+conf+"&nav=1&overview=1&menu=1&loc="+loc+"&tracks="+gtrack+"&hololensid="+sid+"&model="+model+"&binsize="+bin+"&organism="+conf;
				//jumping to genome view
			//	window.location.href="/deltaar/pages/visualization/physical_view.jsp?conf="+conf+"&loc="+loc+"&tracks="+track+"&model="+model+"&bin="+bin+"&zoom=146&hololensid="+sid;
