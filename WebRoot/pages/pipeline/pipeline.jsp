	...
	<div id="content">
	  <div id="right-column1" style="margin-left:10px;width:20%;margin-top:10px;">
			...
			<ul>
				...
				<li><a href="#tad">2. Parameters for TAD</a></li>
				<li><a href="#3dmodel">3. Parameters for 3D model</a></li>
			
			</ul>	
			<div class="block"><img src="/circosweb/images/breadcrumb.gif" />Test Data</div>
			<div class="header_content">
				<b>Organism:</b> hg19<br/> <b>Cell line:</b> GM12878<br/><b>Bin size:</b> 50,000<br/> 
				<b>First bin:</b><br/>chr11:4,500,000-4,550,000<br/>
				<b>Last bin:</b><br/>chr11:6,500,000-6,550,000<br/>
				<b>Download: </b><a href="/circosweb/download/11_50000_matrix.tar.gz">matrix file</a>
			</div>	
			<div class="block"><img src="/circosweb/images/breadcrumb.gif" />Demo Result</div>
			<div class="header_content"><a target="_blank" href="/pipeline/showResult.action?jobid=1499137860032">Show Demo</a></div>
				...
				<div class="header_content" style="position:relative;">
					<table width="893" cellpadding="5" cellspacing="0">
						<tbody>
						     ...
						     <tr>
						       <td colspan="2" id="idrowmatrix">Upload  contact matrix file<span style="color:red;">*</span>&nbsp;&nbsp; <span id="idrowmatrixerr" style="color:red;display:none">The upload contact matrix file can not be empty</span></td>
							</tr>
				 ...
				 &nbsp;&nbsp; &nbsp;Bin Size: the bin size in the input contact matrix. If the uploaded file has high resolution,we suggest you compute an interested region instead of the whole chromosome.<br/>
				 &nbsp;&nbsp; &nbsp;Start Bin: the absolute bin index of start position, can be computed by chromsome position/binsize. <br/>
				 ...
				 <div class="header_content" style="position:relative;">
					...
					<tbody>	
							...
							<tr>
							<td>Cell Line</td>
							<td><input type="text" style="width:200px;" name="pipeBean.cellline" value="GM12878" /></td>
							</tr>
...
robustly detected, p and q are given in units of bins</td>
					...
					<div class="instruction"><a href="http://www.people.fas.harvard.edu/~junliu/BACH/" target="_blank">BACH</a> / <a href="http://calla.rnet.missouri.edu/mogen/" target="_blank">MOGEN</a> / <a href="http://ouyanglab.jax.org/hsa/" target="_blank">HSA</a> is used to compute the spatial structure of the uploaded file.</div>
					<div class="header_content">
						Choose Predict Software&nbsp;&nbsp;&nbsp;&nbsp;<select id="idChooseModel" name="pipeBean.physicalModel" style="width:200px;" onchange="choosePModel()"><option value="BACH">BACH</option><option value="MOGEN">MOGEN</option>
					...
					<div id="idBACHParams" class="header_content">
						<table width="952" cellpadding="5" cellspacing="0">
							<tbody>
								...
								<tr><td colspan="2"><input type="radio" name="pipeBean.featureFileType" value="1" id="idembedfeature" checked="checked" onclick="showBACHFeature()">Use Delta preprocessed genomic feautre file  <input type="radio" name="pipeBean.featureFileType" id="idbachfeature" value="2" onclick="showBACHFeature()">Upload own genomic feature file</td></tr>
								<tr id="iddeltaenzyme"><td>enzyme used</td><td><select name="pipeBean.enzyme" style="width:200px;">
								<option value="HindIII">HindIII</option>
								<option value="MboI" selected="selected">MboI</option>
								<option value="NcoI">NcoI</option>
								</select></td></tr>
								<tr id="iddeltareadslen"><td>reads length of sequencing</td><td><input value="101" name="pipeBean.readslen" style="width:200px;" /></td></tr>
					  ...
					  <div id="iddeltafeature" style="display:none;">
					  	 <div>Upload genomic feature file  <span style="color:red;">*</span>&nbsp;&nbsp;<span id="idrowfeatureerr" style="color:red;display:none">The Upload genomic feature file can not be empty</span></div>
					 	 <iframe src="/circosweb/pages/pipeline/fileupload.jsp?flag=2&amp;jobid=<%=jobid%>" id="idfeaturefile" width="500px" height="80px">
						 </iframe>
					   </div>
					</div>
					...
					<div id="idhsaparams" style="display:none;">
					
					<table width="952" cellpadding="5" cellspacing="0">
							<tbody>
								<tr><td><strong>HSA Parameters:</strong></td></tr>
								<tr><td>Maximum iteration numbers(Maxiter)</td><td><input type="text" value="50" name="pipeBean.hsamaxiter" style="width:200px;" /></td></tr>
								<tr><td>Epslon</td><td><input type="text" value="0.001" name="pipeBean.hsaepslon" style="width:200px;" /></td></tr>
...
<script language="javascript" type="text/javascript">
	...
	function checkEmpty(){
		 ...
		 if($("#idbachfeature").prop("checked")){
		 	var feature = $("#idfeaturefile").val(); 
			if(feature.length == 0 ){
				$("#idrowfeatureerr").css("display","block");
			}else{
				$("#idrowfeatureerr").css("display","none");
			}
		 	
		 }
		 
		 if($("#idusetest").prop("checked") == false){
		 	var matrix = $("#idmatrixfile").val(); 
			if(matrix.length == 0){
				$("#idrowmatrixerr").css("display","block");
			}else{
				$("#idrowmatrixerr").css("display","none");
			}
		 }
	...
	function checkUseTestData(){
		if($("#idusetest").prop("checked")){
		    ...
		    document.getElementById("idfeaturefile").value= "/share/backup2/deltabakcup/hic/testdata/test_feature.txt";
			document.getElementById("idmatrixfile").value="/share/backup2/deltabakcup/circosweb2016/circosweb/download/11_50000.matrix";
		}else{
...
function showBACHFeature(){
	if($("#idembedfeature").prop("checked")){
		$("#iddeltaenzyme").css("display","");
		$("#iddeltareadslen").css("display","");	
		$("#iddeltafeature").css("display","none");	
	}
	else if($("#idbachfeature").prop("checked")){
		$("#iddeltaenzyme").css("display","none");
		$("#iddeltareadslen").css("display","none");	
		$("#iddeltafeature").css("display","block");	
	}
