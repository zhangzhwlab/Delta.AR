 ...
 */
{
    ...
    _trackMenuOptions: function() {
        ...
        return [
			...
			{ label: 'Show in Delta.AR',
              type: 'dijit/CheckedMenuItem',
              title: "checked to show this track in Delta.AR",
              checked: that.isShowAR(),
              ...
              onClick: function() {
				...
				var curtrack = that.name;
				var srctrack = curtrack;
				
				var deltrack="";
				if(curtrack.indexOf("signal") > -1){
			...
			//	alert("loc="+loc);
				if(loc != null && loc != ""){
									...
									chrtmp = loc.substring(0,index1);
									pos_starttmp = loc.substring(index1+1,index2);
									pos_endtmp = loc.substring(index2+2,loc.length);
									//identify genome range , it at least show 5 atoms  and at more show 
								...
								//	alert("atomcount="+atomcount);
									if(atomcount > 60 || atomcount<5){
										...
										if(this.checked ){
															...
															$.ajax({
																		...
																		success : function(data){
																			...
																			if(deltrack!="" && deltrack.length > 0 ){
																				...
																				JBrowse.updateTracksARFlag(deltrack);					
																			}
										  ...
										  //remove track from url
											if(artracks[srctrack] != null){
												deltrack =srctrack;
												delete artracks[srctrack] ;
												...
												$.ajax({
																		...
																		success : function(data){
																			if(deltrack!="" && deltrack.length > 0 ){
																				that.setShowAR(false);				
																			}
																			console.log("Genome View Update Unshow.AR Success");
																		},
																		error : function (){
																			console.log("Genome View Update Unshow.AR Error");
																			//alert("Update AR Error");
																		}
												});
    ...
    renderRegionHighlight: function( args, highlight ) {
		...
		var rspan = right-left;
		var hightleft = left;
		if(rspan <0 ){
			hightleft =  args.leftBase;
		}
		
		
        //var width = (right-left)*100/block_span;
        //left = (left - args.leftBase)*100/block_span;
		var width = (right-hightleft)*100/block_span;
        left = (hightleft - args.leftBase)*100/block_span;
		
		console.log("===============width="+width+",args "+args.leftBase+","+args.rightBase+","+block_span+",highlight="+hightleft+","+right+","+rspan+",block left="+left);
