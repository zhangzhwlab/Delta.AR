...
constructor: function( args ) {
	...
	this.browser.subscribe( '/jbrowse/v1/c/tracks/showar',     dojo.hitch( this, 'showARTracks' ));
    this.browser.subscribe( '/jbrowse/v1/c/tracks/unshowar',   dojo.hitch( this, 'unShowARTracks' ));
...
dragEnd: function(event) {
	...
	//
	var v_start = this.minVisible();
	var v_end = this.maxVisible();
	$("#idembed4cexpandStartRegion").val(v_start);
	$("#idembed4cexpandEndRegion").val(v_end);
	var virtual4cpanel =document.getElementById("virtualpanel"); //virtualpanel
	if(virtual4cpanel != null && virtual4cpanel.style.display != "none"){
		var virtual4cc =document.getElementById("id4ccanvas"); 
	

			if(virtual4cc!= null ){ //loadVirtual4C
				embedVirtual4CUpdate();
			}
...
setLocation: function(refseq, startbp, endbp) {
	...
	//console.log("==================Genome View=====pxPerBp================="+this.pxPerBp+","+endbp+","+","+startbp+",this.maxPxPerBp "+this.maxPxPerBp );
...
centerAtBase: function(base, instantly) {
    ...
    if (instantly) {
		...
		this.trackIterate(function(track) { track.clear(); });
		 this.showCoarse();// first show position , update by tangbx
		 
		this.showVisibleBlocks(true); // then show blocks
...
onCoarseMove: function( startbp, endbp ) {
	...
	//console.log("===============onCoarseMove"+startbp+","+endbp+",this width="+this.getWidth());
...
updateLocationThumb: function() {
	...
	//console.log("===========updateLocationThumb"+startbp+","+endbp+"================");
...
zoomUpdate: function(zoomLoc, fixedBp) {
	...
	this.showCoarse(); //first update location
	
    this.showVisibleBlocks(true); //then update tracks
    this.showDone();
	
	//if virtual 4c show,then redraw
	//show start,end
	var v_start = this.minVisible();
	var v_end = this.maxVisible();
	var chrtmp = $("input[name='refseq']").val();
	//here,identify genome region
	...
	var pos_starttmp = parseInt(v_start)+1;
	...
	if(atomcount >=5 && atomcount<=60){
		...
		$('<div></div>').appendTo('body')
							  .html('<div style=\"margin-top:10px;\">Update Hololens with this genome region?</div>')
							  .dialog({
									  ...
									  Ok: function(){
											...
											$.ajax({
													...
													success : function(data){
														...
														console.log("Genome View Update Show.AR Success");
													},
													error : function (){
																												
														//alert("Update AR Error");
														console.log("Genome View Update Show.AR Error");
													}
											});
...
showTracks: function( trackConfigs ) {
    ...
    var needed = dojo.filter( trackConfigs, function(conf) {
		var trackname = conf.label;		
		
		var  track = this._getTracks( [conf.label] );
		...
		if(tracks != null && tracks.length > 0 ){
								if(tracks.indexOf(",") > -1 ){
									...
									for (var j=0;j<needed.length;j++){
										var curtrack=needed[j].label; 
										var srctrack = curtrack;
										if(curtrack.indexOf("_signal")>-1){
										...
										for(var ti=0;ti<trass.length;ti++){
											var t_tr = trass[ti];
											if(t_tr == curtrack){
												needed[j].showar = true;
												flag = 1;
												if(artracks[srctrack] ==null){
													artracks[srctrack] = srctrack;
												}
												break;
											 }
										  }
										  if(flag == 0){
												needed[j].showar =false; 
										  }
										}
...
showARTracks: function( /**Array[String]*/ trackNames ) {
	
	alert("showARTracks"+trackNames);
    var tracks = this._getTracks( trackNames );
    array.forEach( tracks, function( track ) {
                       track.setShowAR(true);
                   });
 ...
 * names of tracks that were actually unpinned.
 */
unShowARTracks: function( /**Array[String]*/ trackNames ) {
	alert("hideARTracks"+trackNames);
    var tracks = this._getTracks( trackNames );
    array.forEach( tracks, function( track ) {
                       track.setShowAR(false);
                   });
...
callHololens :function (){
	...
	var loc = document.getElementById("idpos").value;
//alert(loc);
	if(loc != null && loc != ""){
		...
		//	alert(chrtmp+","+pos_starttmp+","+pos_endtmp);
			/*var threfurl="http://deltaar.big.ac.cn/deltaar/ar/changex.action?jobid="+organism+"&modelname="+model+"&binsize="+binsize+"&species="+organism+"&track="+jumptrack+"&chrom="+chrtmp+"&start="+pos_starttmp+"&end="+pos_endtmp+"&hololensid="+hololensid;
			...
			$.ajax({
						...
						data : params,
						success : function(data){
							console.log("Genome View Update AR Success");
						},
						error : function (){
