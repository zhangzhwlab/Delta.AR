                ...
                thisB.loadUserCSS().then( function() {
                    ...
                    thisB.loadRefSeqs().then( function() {
                       ...
                       thisB.initView().then( function() {
                        ...
                        // console.log("=============alwaysOnTracks"+thisB.config.alwaysOnTracks);
						// console.log("======================forceTracks ="+thisB.config.forceTracks);
						
						   
						   if(thisB.cookie("recentTracks")){tracksToShow = tracksToShow.concat(thisB.cookie("recentTracks").split(",")); }
                       //   console.log("============"+thisB.cookie("recentTracks"));
...
initView: function() {
	...
	/*	dojo.create('a', {
				...
				innerHTML: '<img class="logo" src="'+this.resolveUrl('img/delta.png')+'">',
				title: 'Delta Home',
				href: 'http://deltaar.big.ac.cn'
		}, menuBar );*/
        ...
        if( this.config.show_nav ) {
			...
			this.addGlobalMenuItem( 'view', new dijitMenuItem({
                ...
                onClick: function() {
					...
					if (r != null) dataset= unescape(r[2]);
					if(dataset != "" && dataset.indexOf("custom") <0 ){
						window.location.href = "http://"+window.location.host+"/jbrowse/index.html?data="+dataset+"_custom";
					}else{
						window.location.href = "http://"+window.location.host+"/jbrowse/index.html?data="+dataset;
					}
		...
		//add a topological view link
		/*menuBar.appendChild( new dijitMenuItem({
           label: "Topological View",
           iconClass: "dijitEditorIcon dijitEditorIconCut",
           onClick: function(){alert('i was clicked')}
		}) );*/
		
	/*var topobutton = new dijitButton({
            className: 'topoview',
            innerHTML: 'Topological View',
            onClick: function() {
               alert('click me');
            }
        }
    );
	menuBar.appendChild(topobutton.domNode);*/

	/*	this.addGlobalMenuItem( 'Topological View',
                                    new dijitMenuItem(
                                        {
                                            id: 'menubar_topo',
                                            label: 'Topological View',
                                            iconClass: 'jbrowseIconTopo',
                                            onClick: function(){alert('i was clicked')}
                                        })
                                  );

								   
         this.renderGlobalMenuCustom( 'Topological View', {}, menuBar );
		*/
...
renderGlobalMenuCustom: function( menuName, args, parent ) {
    this.afterMilestone( 'initView', function() {
        var menu = this.makeGlobalMenuCustom( menuName );
       /* if( menu ) {
            args = dojo.mixin(
                {
                    className: menuName,
                    innerHTML: '<span class="icon"></span> '+ ( args.text || Util.ucFirst(menuName)),
                    id: 'menubutton_'+menuName,
					onclick: function(){
						alert("click");
					}
                },
		...
		//alert(menu.domNode);
	     //dojo.addClass( menu.domNode, 'menu' );
		 parent.appendChild( menu.domNode );
    },this);
},

makeGlobalMenuCustom: function( menuName ) {
    var items = ( this._globalMenuItems || {} )[menuName] || [];
    if( ! items.length )
        return null;

    var menu = new dijitMenu({ id: 'dropdownmenu_'+menuName });
...
navigateToLocation: function( location ) {
	...
	//	console.log("=============get the location="+location.start+","+location.end);
...
showTracks: function( trackNames ) {
    this.afterMilestone('initView', dojo.hitch( this, function() {
        ...
        // publish some events with the tracks to instruct the views to show them.
         this.publish( '/jbrowse/v1/n/tracks/visibleChanged' ); //change order
		this.publish( '/jbrowse/v1/c/tracks/show', trackConfs );
		
    }));
},
//add by tangbx 20181206
updateTracksARFlag:function(trackNames){
	if( typeof trackNames == 'string' )
            trackNames = trackNames.split(',');
		...
		if(trackConfs.length>0){
			for(var i=0;i<trackConfs.length;i++){
				var curtrackconf = trackConfs[i];
				console.log(curtrackconf.label+",curtrackconf.showar  "+curtrackconf.showar);
				curtrackconf.showar = false;
			}
			
		}
		console.log("updateTracksARFlag");
		console.log(trackConfs);
		//first hide
		this.publish( '/jbrowse/v1/c/tracks/hide', trackConfs );
		this.publish( '/jbrowse/v1/v/tracks/hide', trackConfs );
        this.publish( '/jbrowse/v1/n/tracks/visibleChanged' );
		this.publish( '/jbrowse/v1/n/tracks/redraw' );
		//then show
		this.publish( '/jbrowse/v1/n/tracks/visibleChanged' );
		this.publish( '/jbrowse/v1/c/tracks/show', trackConfs);
...
*hide given tracks
********************/
hideTracks: function( trackNames ){
	    this.afterMilestone('initView', dojo.hitch( this, function() {
        if( typeof trackNames == 'string' ){
			if(trackNames.indexOf(",") > -1){
				trackNames = trackNames.split(',');
			}
		}
...
createNavBox: function( parent ) {
	...
	this.goTopologicalButton = new dijitButton(
    {
        label: 'Goto Topological',
        onClick: dojo.hitch( this, function(event) {
           if(this.config.show_embedtopo){
			   gotoEmbedTopology();
		   }else{
			     goTopology(); // go to a new window topological function
		   }
         
            dojo.stopEvent(event);
        }),
        id: 'search-gotopological-btn'
	...
	this.goPhysicalButton = new dijitButton(
		{
			label: 'Goto Physical',
			onClick: dojo.hitch( this, function(event) {
			
			   goPhysical(); // go to topological function
				dojo.stopEvent(event);
			}),
			id: 'search-gophysical-btn'
	}, dojo.create('button',{},navbox));
		
		
	
	this.goVirtual4cButton = new dijitButton(
    {
        label: 'Virtual 4C',
        onClick: dojo.hitch( this, function(event) {
			showVirtual4C();
        ...
        id: 'search-gotovirtual4c-btn'
    }, dojo.create('button',{},navbox));
	
		
	this.exportPdfButton = new dijitButton(
    {
        label: 'Export',
        onClick: dojo.hitch( this, function(event) {
            
           savePageAs(); // go to topological function
            dojo.stopEvent(event);
        }),
        id: 'search-goexport-btn'
    }, dojo.create('button',{},navbox));
	
	  var exportwait = document.createElement("img");
    //bigZoomOut.type = "image";
    exportwait.src = this.resolveUrl( "img/spinner.gif" );
    exportwait.id = "exportwait";
    exportwait.style.display="none";
    navbox.appendChild(exportwait);
	
	
	//add search gene box
	var searchgeneboxSpan = dojo.create('span', {
        'id':'search-gene-box',
        'class': locationMode
    }, navbox );
	this.searchGeneBox = new dijitTextBox(
        {
            id: "geneid",
            name: "geneid",
			value: "HBB",
            style: { width: '15ex' },
            maxLength: 400,
            searchAttr: "gene name",
            title: 'Enter a gene name to search'
        },
    dojo.create('input', {}, searchgeneboxSpan) );
	
  this.searchgeneButton = new dijitButton(
    {
        label: 'Search Gene',
        onClick: dojo.hitch( this, function(event) {
            addSearchGene(this.searchGeneBox.get('value')); //used to search gene 
            dojo.stopEvent(event);
        }),
        id: 'search-gene-btn'
    }, dojo.create('button',{},searchgeneboxSpan));
...
setMultipleHighlightAndRedraw: function( location ) {
    location = this.regularizeLocation( location );
