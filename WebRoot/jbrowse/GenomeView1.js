...
showTracks: function( trackConfigs ) {
	...
	//add by tangbx , 20181122
	dojo.filter( trackConfigs, function(conf) {
		var trackname = conf.label;
		 if(trackname.indexOf('signal') > -1){
					trackname = trackname.substr(0,trackname.length-7);
		} 
		this.checkDefaultHololensTracks(trackname);
...
checkDefaultHololensTracks:function(curtrack){
	var hololensid =  getQueryString("hololensid");
	if(hololensid == null || hololensid==""){	
		hololensid = Cookies.get("delta.ar.hololensid");
	} 
