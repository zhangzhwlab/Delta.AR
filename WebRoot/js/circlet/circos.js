...
var artracks={}; // this is used to store tracks show in delta.AR,added 1207


var MIN_WINDOW_SIZE=800;
var MAX_WINDOW_SIZE=1800;
var MIN_RADIUS_SIZE=20;
var MAX_RADIUS_SIZE=200;
...
var windowsize_vertical;
var radiussize_vertical;
...
function loadLayout(){//ideogramwidth:14,
	...
	mylayout.sizePane("west", "1%");
	mylayout.sizePane("center", "88%");	
	mylayout.hide('west');*/
	...
	var i_track_number = Cookies.get("topo_maxtrack");
	if(i_track_number == null ){
		MAX_TRACK_COUNT = 5;
	}else{
		MAX_TRACK_COUNT = i_track_number;
	}
	...
	windowsize_vertical = $("#windowsize-vertical").slider({
	...
	var windowlabel = "<p id='idwinmax' style='font-size:10px;padding-top:0px;margin-left:0px;padding-left:10px;'>1800</p>";
	$("#windowsize-vertical").append(windowlabel);
	windowlabel = "<p id='idwinmin' style='font-size:10px;padding-top:170px;margin-left:0px;padding-left:15px;'>800</p>";
	$("#windowsize-vertical").append(windowlabel);
	...
	radiussize_vertical = $( "#radiussize-vertical" ).slider({
      orientation: "vertical",
      range: "min",
      min: MIN_RADIUS_SIZE,
	  step: 10,
	...
	windowlabel = "<p id='idradiusmax' style='font-size:10px;padding-top:0px;margin-left:0px;padding-left:13px;'>200</p>";
	$("#radiussize-vertical").append(windowlabel);
	windowlabel = "<p id='idradiusmin' style='font-size:10px;padding-top:70px;margin-left:0px;padding-left:15px;'>20</p>";
	$("#radiussize-vertical").append(windowlabel);
...
function config_getConfig(){
	...
	var config_size=[MAX_TRACK_COUNT];
	return config_size;
}


function modify_config_setConfig(e){
	...
	MAX_TRACK_COUNT = e;
	//Cookies.set("topo_maxtrack", MAX_TRACK_COUNT, { path: 'topo' }) ;	
}
...
function getJumpLink(jumpchr,jumpstart,jumpend,jumpkey){
	...
	var opkey = jumpkey;
		...
		if(conf != null){
			...
			link_physical="http://"+window.location.host+"/deltaar/pages/visualization/physical_view.jsp?conf="+conf+"&loc="+jumpchr+"%3A"+jumpstart+".."+jumpend+"&tracks=3dmodel,"+opkey;
		}else{
			link_genome="http://"+window.location.host+"/jbrowse/index.html?data=human&loc="+jumpchr+"%3A"+jumpstart+".."+jumpend+"&tracks="+opkey;
			if(opkey == "transcript"){
				opkey = "ensembl_gene";
			}
			link_physical="http://"+window.location.host+"/deltaar/pages/visualization/physical_view.jsp?loc="+jumpchr+"%3A"+jumpstart+".."+jumpend+"&tracks="+opkey;
		}
...
function clearCookies(rpath){	
	...
	Cookies.remove(rpath+'_maxtrack', { path: rpath });
	Cookies.remove(rpath+'_maxselecttrack', { path: rpath });
}
...
//this  used to user click by "Show Virtual4C " button
function dialogLoadVirtual4C(){
		...
		var p_pos = $("#curpos").val(); 
			...
			if(p_pos!=null){
				if(p_pos.indexOf("..")>-1){
					...
					pos_end = p_pos.substring(idex+2,p_pos.length);
					var idex1 = p_pos.indexOf(":");
					pos_start = p_pos.substring(idex1+1,idex);
					p_pos = p_pos.substring(0,idex);
				}
			}

			
			var htmltext="<table cellpadding=\"5\" cellspacing=\"0\" style=\"margin-top:5px; font-size:13px;\"><tbody><tr><td width=\"230\"><b>Organism</b></td></tr>";
			htmltext += "<tr><td><select id=\"id4orga\" style=\"width:120px;\" onchange=\"showCell()\"><option value=\"hg18\">Human (hg18)</option><option value=\"hg19\">Human (hg19)</option></select></td></tr>";
					  
			htmltext += "<tr><td><b>Dataset</b></td></tr><tr><td><select style=\"width:200px;\" id=\"id4dataset\" onchange=\"showDataset()\" ><option value='GSE18199_GM06690' selected='selected'>GSE18199_GM06690</option><option value='GSE18199_K562'>GSE18199_K562</option><option value='GSE35156_IMR90'>GSE35156_IMR90</option><option value='GSE35156_IMR90'>GSE35156_IMR90</option></select></td></tr>";
			htmltext += "<tr><td><b>Resolution Size</b></td></tr><tr><td><select id=\"id4resolution\" style=\"width:80px;\"><option value='1000000'>1mb</option><option value='100000'>100kb</option></select></td></tr>";
			htmltext += "<tr><td><b>Position</b><span style=\"color:red;\">*</span></td></tr><tr><td><input type=\"text\" id=\"id4position\" value=\""+p_pos+"\" style=\"width:120px;\" /></td></tr>";
			htmltext += "<tr><td><b>View region</b></td></tr><tr><td>Start<input type=\"text\" id=\"id4start\" value=\""+pos_start+"\" style=\"width:120px;\"></td></tr><tr><td>End<input type=\"text\" id=\"id4end\" value=\""+pos_end+"\" style=\"width:120px;\"></td></tr>";	  
			 ...
			 $('<div></div>').appendTo('body')
							  .html(htmltext)
							  .dialog({
								  modal: true, title: 'Virutal 4C parameters', zIndex: 10000, autoOpen: true,
								  width: 'auto', resizable: false,						 
								  close: function (event, ui) {
									  //remove from cookie	
									  $(this).remove();
								  ...
								  buttons:{
									  Ok: function(){
									    pdataset = $("#id4dataset").val(); 
										expandRegion = $("#id4expand").val(); 
										presolution = $("#id4resolution").val(); 
										ploc = $("#id4position").val(); 
										pos_start = $("#id4start").val();
										pos_end = $("#id4end").val();
										...
										start_bin = 0;
										if(pdataset == urlconf ){
											start_bin = getQueryString("sb"); 
										}
										$("#idembed4cdataset").val(pdataset);
										$("#idembed4cbin").val(presolution);
										$("#idembed4cpos").val(ploc);
										$("#idembed4cstartbin").val(start_bin);
										$("#idembed4cexpandStartRegion").val(pos_start);
										$("#idembed4cexpandEndRegion").val(pos_end);
										...
										canvastt.addEventListener("mousemove",extend_canvas_mouse_move2,false);
										
										
										draw4cplotfunccirclet(pdataset,presolution,ploc,pos_start,pos_end,start_bin);	
									    $(this).remove();
...
//this is used to open vitual 4c dual mode with physical view
function loadVirtual4C(){
		...
		var p_pos = $("#curpos").val(); 
		 ...
		 pdataset = window.parent.document.getElementById("4cdataset").value;
	    presolution = window.parent.document.getElementById("4cbin").value;
		ploc = window.parent.document.getElementById("4cpos").value;
		start_bin = window.parent.document.getElementById("4cstartbin").value;
		expandRegion = parseInt(presolution)*20; 	
