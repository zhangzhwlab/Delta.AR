	...
	if(t_toomany ==0 ){
		...
		var ajaxurl = "";
			if(trackobj.storeclass == "GFF3" || trackobj.storeclass=="gff3"){
				ajaxurl="/deltaar/ajax/getChromatinInterData.action";
			}else if(trackobj.storeclass == "mysql"){
				ajaxurl="/deltaar/ajax/getChromatinInterDataFromTable.action";
			}
			...
			$.ajax({
					...
					success:function(value){
						...
						if(interacionData!=null || interacionData!=undefined ){
							...
							var left_margin = canvas_width/2+track_radius-labelmargin;//-10
	...
	for(i=0;i<region_lst.length;i++){
			...
			if(t_statbin >1 && trackobj.statisFile != null){ // statis file
			 var t_statisFile = trackobj.statisFile;
				...
				$.ajax({
				   url:'/deltaar/ajax/getHistogramStatics.action',
				   type:'post',
				   dataType:'json',
				   data:params,
				   success:function(value){
				...
				$.ajax({
					   url:'/deltaar/ajax/getQualityData.action',
					   type:'post',
					   dataType:'json',
					   data:params,
					   success:function(value){
...
function ajax_getTabixGff3Data(trackobj){
		...
		if(region_lst.length == 1){
			...
			imageObj.src="http://"+window.location.host+"/deltaar/images/loader.gif";
		}
		...
		*/
		...
		for(i=0;i<region_lst.length;i++){
			...
			if(t_statbin > cur_toomany && trackobj.statisFile != null){ // statis file
				...
				$.ajax({
					   url:'/deltaar/ajax/getTabixGff.action',
					   type:'post',
					   dataType:'json',
					   data:params,
					   success:function(value){		
		...
		for(i=0;i<region_lst.length;i++){
			...
			if(t_statbin > cur_toomany && trackobj.statisFile != null){ // statis file
				...
				if(trackobj.storeclass =="GFF3" || trackobj.storeclass == "gff3"){
					...
					//url = "/deltaar/ajax/ajaxFindGivenScopeGene.action";
					url = "/deltaar/ajax/ajaxFindTranscriptExon.action";
				}
				$.ajax({
					   url: url,
					   type:'post',
