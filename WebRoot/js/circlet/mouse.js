...
function horizon_ideogram_make(event)
{
	...
	*/
	...
	if(synphysical == 1){
		indicator.style.top = (pos[1]- 45)+"px";
	}else{
		indicator.style.top = (pos[1]- 92)+"px";
	}
			...
			if(contextmenudiv.length<=0){
				...
				if(contextmenutext_div != null){
					...
					if(conf != null){
					   ...
					   link_physical="http://"+window.location.host+"/deltaar/pages/visualization/physical_view.jsp?conf="+conf+"&loc="+chr+"%3A"+ideogram_start+".."+ideogram_end+"&tracks=3dmodel,"+opkey;
					}else{
						...
						if(opkey == "transcript"){
						 ...
						 link_physical="http://"+window.location.host+"/deltaar/pages/visualization/physical_view.jsp?loc="+chr+"%3A"+ideogram_start+".."+ideogram_end+"&tracks="+opkey;
					}
