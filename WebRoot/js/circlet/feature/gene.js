			...
			for(var j=0;j<gff.children.length;j++){
				...
				if(j>0 ){
					...
					if(strand=="+"){ // current end connect to previous start
						...
						if(gff.name=="ENST00000532108"){
							var d_test_start=arc_offset+pre_end_arc;
							var d_test_end = arc_offset+cur_start;
							
							console.log(j+","+pre_end+","+child_start+","+d_test_start+","+d_test_end);
						}
						
						ctx1.arc(centerx,centery,current_radius,arc_offset+pre_end_arc, arc_offset+cur_start,false);
						*/
						var presubchildren = gff.children[j-1];
						pre_start = parseInt(presubchildren.start);
						pre_start_arc = (pre_start - region_start) * sf;	
												
						cur_end = child_arc_end;	
						ctx1.lineWidth = 1;
						ctx1.strokeStyle= "black";
						ctx1.beginPath();
						
						if(gff.name=="ENST00000348423"){
							var d_test_start=arc_offset+pre_start_arc;
							var d_test_end = arc_offset+cur_end;
							
							console.log(j+","+pre_start+","+child_end+","+d_test_start+","+d_test_end);
						}
						
						ctx1.arc(centerx,centery,current_radius,arc_offset+pre_start_arc, arc_offset+cur_end,true);
