		...
		$.ajax({
			...
			success : function(data){
			    ...
			    $.get(data.speciesJson,function(result){
					...
					params={"organism":curorganism,"param1":"cncb"};	
					$.ajax({
						url : '/deltaar/ajax/ajaxGetPhysicalMetaDataFromTable.action?time='+new Date().getTime(),
						type : 'post',
						dataType : 'json',
						data : params,
						async: false,
						success : function(data){
							if(data.metaphysicalList != null){
								createCNCBCategory(data.metaphysicalList);	
							}
						},
						error : function(){
							console.log("load cncb data error!");
						}
					});
...
function choose3DmodelFunc(flag){
	...
	//need to add cncb dataset
	function createCNCBCategory(value){
		var div = $("<div>",{
		css:{
			'background' : '#ffffff',
		...
		var cindex =3000+ 1 *10 ;
		var table=$("<table>").attr("cellspacing","0").css("display","none").attr("id","panel_cncb"+cindex);
		var tr;
		var td;
		...
		for(var i=0;i< value.length;i++){
			...
			tdval="<td ><input type=\"checkbox\" id=\"tlst_"+track.label+"\" value=\""+track.dataCategory+","+track.label+",white\" onclick=\"toggleTrack('"+track.dataCategory+"','"+track.label+"',"+cindex+",1)\">"+track.label;  
		...
		console.log("createCNCBCategory");
	}
