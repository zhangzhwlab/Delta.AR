...
function drawPeakTrack(trackobj){
	...
	if(storageclass=="GFF3" || storageclass=="gff3"){
		ajaxurl="/deltaar/ajax/ajaxGetGPeakff3file.action";		
	}else if(storageclass=="mysql"){
		
		ajaxurl="/deltaar/ajax/ajaxGetGPeakFromTable.action";
	}
	...
	$.ajax({
				...
				success : function(data){
					...
					if(data.gffList != null){
							//console.log("peak data length="+data.gffList.length );
									...
									//	console.log("peak index = "+anchoratomindex+","+targetatonindex);
										...
										if(anchoratomindex > -1 && targetatonindex > -1 && anchoratomindex != targetatonindex && anchoratomindex< atoms.length && targetatonindex< atoms.length){
											...
											if(glviewer != null){
												...
												var cylinder = glviewer.addCylinder({start: b1,end: e1,radius:0.01, tocap:true,color:"grey",alpha:0.1,anchor:anpos,target:tarpos,clickable:true, callback: function(){	
												
													//interaction peak													
													//this.color='white';
													//this.tocap= false;
													//this.fromcap= true;
													//this.color.setHex(0x00FF0000);
													//this.alpha=0.9;
													this.updateStyle({color:"red",alpha:0.9});
													glviewer.render();	
													//alert("===cancel set alpha==");										   
													SELECTPEAK = this;
													var jump_pos = "";
...
function drawTADSurfaceTrack(trackobj){
	...
	if(storageclass=="GFF3" || storageclass=="gff3"){
			...
			$.ajax({
				url : '/deltaar/ajax/ajaxGetGff3file.action',
				type : 'post',
				dataType : 'json',
				data : params,
				async: false,
				success : function(data){
					if(data.gffList != null){
						console.log("tad size="+data.gffList.length);
						if(data.gffList.length>200){
							...
							if(gffobj != null ){
								...
								console.log("tad="+tadstart+","+tadend);
...
function btnShow4Cplot(){
	
	var t_loc = getQueryString("loc");
		var startregion=0;
		var endregion = 0;
		var tpchr = "";
		if(t_loc!=null){
			if(t_loc.indexOf("..")>-1){
				var idex = t_loc.indexOf("..");	
				endregion = t_loc.substring(idex+2,t_loc.length);
				var idex1 = t_loc.indexOf(":");
				if(idex1 > -1 ){
					startregion = t_loc.substring(idex1+1,idex);
					tpchr = t_loc.substring(0,idex1);
				}
	...
	show4CPlot(tpchr,startregion,0);
}
//this is used to show 4c plot
function show4CPlot(pchr,pstart,pend){
	...
	//if(presolution/1000000 > 0 ){ //mb
		expandRegion = presolution*20;
	//}	
	var ploc= pchr+":"+pstart;
	$("#4cpos").val(ploc);
	...
	//this is used to  show genome view
	if($("#gviewid").prop("checked") == false){
		$("#gviewid").prop("checked","checked");
	}	

	//dailog expand region
	//var startregion = pstart - expandRegion;
	//var endregion = pstart + expandRegion;
	...
	showGenomeViewWithPosition(startregion,endregion);
}
