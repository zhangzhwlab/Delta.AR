...
import cn.ac.big.circos.po.EnclosePhysicalBean;
...
public class PhysicalViewAction extends ActionSupport{
	...
	//	System.out.println(filepath);
		String memkey= "physical_"+this.curDataset+"_new";
		
		MemCachedClient memcacheclient = BIGMemCachedClient.getMemCacheClient();
		if(memcacheclient.get(memkey) != null ){
			
			EnclosePhysicalBean ep =  (EnclosePhysicalBean)memcacheclient.get(memkey);
			
			categoryList = (List<CategoryTrack>)ep.getCategoryList();
			this.speciesJson = ep.getSpeciesJson();
			this.organism = ep.getOrganism();
		}else{
				try{
					File file = new File(filepath);
					if(file.exists() == true ){
						...
						while((line = br.readLine()) != null ){
							...
							if(line.startsWith("[") && line.endsWith("]")){
								...
								while((line = br.readLine())!=null && (line.length()>0)){
									...
									}else if(arrs[0].startsWith("label")){
										track.setLabel(arrs[1]);
									}
									/*if(organism!=null){
						...
						EnclosePhysicalBean ep = new EnclosePhysicalBean();
						ep.setCategoryList(categoryList);
						ep.setOrganism(organism);
						ep.setSpeciesJson(speciesJson);
						memcacheclient.set(memkey, ep);
        ...
        memkey= "physical_selectAllPhysicalModelList";
	
		if(memcacheclient.get(memkey) != null ){
			this.physicalModelList = ( List<PhysicalModelBean>)memcacheclient.get(memkey);
		}else{			
			...
			memcacheclient.set(memkey, physicalModelList);
		}
		...
		try{
			...
			bw.write("conf=conf/physical/"+circosTrack.getDataset()+".conf\n");	
     		...
     		if(circosTrack.getGlyph().equals("3dmodel")){     			
     			...
     			bw.write("key="+circosTrack.getName()+"_custom_3dmodel\n");
     			bw.write("category="+circosTrack.getCategory()+"\n");
     			bw.write("meta=label:"+circosTrack.getName()+"_custom_3dmodel;name:"+circosTrack.getName()+"_custom_3dmodel;data_category:"+circosTrack.getCategory()+";organism:"+circosTrack.getOrganism()+";cell:none;data_type:"+circosTrack.getGlyph()+";lab:none;antibody:none;peaks:none;replicate_number:none;file_type:"+circosTrack.getStoreclass()+"\n");
     			...
     			bw.write("meta=label:"+circosTrack.getName()+";name:"+circosTrack.getName()+";data_category:Custom Annotated Track;organism:"+circosTrack.getOrganism()+";cell:none;data_type:"+circosTrack.getGlyph()+";lab:none;antibody:none;peaks:none;replicate_number:none;file_type:"+circosTrack.getStoreclass()+"\n");
	...
	public int getStart_bin() {
		return start_bin;
	}

	public void setStart_bin(int start_bin) {
